...
void totalWaysChange(long long& totalWays, long long &currOne, long long& currTwo, long long& newFamilyCount) {
	...
	for (long long i = 0;i < tCases;i++) {
		...
		for (long long k = 0; k < m;k++) {
			...
			if (findRoot(nodes[i]) == findRoot(nodes[j])) {
				...
				numPeople--;
				totalWaysChange(totalWays, currOne, currTwo, newFamilyCount);
